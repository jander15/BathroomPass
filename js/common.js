...
// --- Utility Functions ---
function normalizeName(name) { if (typeof name !== 'string') return ''; const idx = name.indexOf('('); return idx > -1 ? name.substring(0, idx).trim() : name.trim(); }
function decodeJwtResponse(token) { const base64Url = token.split('.')[1]; const base64 = base64Url.replace(/-/g, '+').replace(/_/g, '/'); const jsonPayload = decodeURIComponent(atob(base64).split('').map(c => '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2)).join('')); return JSON.parse(jsonPayload); }
function showSuccessAlert(message) { if (alertDiv) { alertMessageSpan.textContent = message; alertDiv.classList.remove("hidden"); setTimeout(() => alertDiv.classList.add('hidden'), 5000); } }
function showErrorAlert(message) { if (errorAlertDiv) { errorAlertMessageSpan.textContent = message; errorAlertDiv.classList.remove("hidden"); setTimeout(() => errorAlertDiv.classList.add('hidden'), 10000); } }
function setupProfileMenu() { if (!profilePicture || appState.ui.isProfileMenuSetup) return; appState.ui.isProfileMenuSetup = true; profilePicture.addEventListener('click', (event) => { event.stopPropagation(); profileDropdown.classList.toggle('hidden'); }); dropdownSignOutButton.addEventListener('click', handleGoogleSignOut); bodyElement.addEventListener('click', () => { if (!profileDropdown.classList.contains('hidden')) { profileDropdown.classList.add('hidden'); } }); }
function populateDropdown(dropdownId, arr, defaultText, defaultValue = "") { let selectElement = document.getElementById(dropdownId); if (!selectElement) return; selectElement.innerHTML = ''; selectElement.add(new Option(defaultText, defaultValue)); arr.forEach(itemValue => { if (itemValue !== defaultText && itemValue !== defaultValue) { selectElement.add(new Option(itemValue, itemValue)); } }); selectElement.value = defaultValue; }
...
async function sendAuthenticatedRequest(payload, isRetry = false) {
    ...
    try {
        ...
        if (!response.ok) { if (response.status === 401 || response.status === 403) throw new Error("SESSION_EXPIRED"); throw new Error(`HTTP error ${response.status}`); }
        ...
        if (error.message === "SESSION_EXPIRED" && !isRetry) {
            ...
            try {
                ...
                if (refreshResponse.result === 'success' && refreshResponse.idToken) { appState.currentUser.idToken = refreshResponse.idToken; return await sendAuthenticatedRequest(payload, true); }
            } catch (refreshError) {}
...
async function fetchAllStudentData() { try { const data = await sendAuthenticatedRequest({ action: ACTION_GET_ALL_DATA }); appState.data.allNamesFromSheet = (data && Array.isArray(data)) ? data : []; } catch (error) { showErrorAlert("Failed to load student data. Please check connection and refresh."); throw error; } }
function populateCourseDropdownFromData() { if (appState.data.allNamesFromSheet.length > 0) { const uniqueClassNames = new Set(appState.data.allNamesFromSheet.map(item => item.Class).filter(c => c)); appState.data.courses = Array.from(uniqueClassNames).sort(); } }
...
function tryInitializeApp() {
    if (isDomReady && isUserReady && !hasInitializationRun) {
        ...
        if (appContent) { appContent.classList.remove('hidden'); appContent.style.display = 'flex'; }
...
async function processSignIn(tokenData) {
    if (tokenData && tokenData.result === 'success' && tokenData.idToken) {
...
async function handleSignIn(authCode) {
    try {
        ...
        if (!await processSignIn(tokenData)) {
            showErrorAlert("Could not complete the sign-in process.");
        }
    } catch (error) {
        showErrorAlert("An error occurred during sign-in.");
    }
}
...
function initGoogleSignIn() {
    ...
    try {
        ...
        customButton.onclick = () => { if (loadingOverlay) loadingOverlay.classList.remove('hidden'); codeClient.requestCode(); };
        googleSignInButton.appendChild(customButton);
    } catch (error) {
        showErrorAlert("Google Sign-In library failed to initialize.");
    }
}
